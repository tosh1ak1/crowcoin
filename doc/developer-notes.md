...
Crowcoin Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- ThreadSocketHandler : Sends/Receives data from peers on port 9630.
...
- ThreadRPCServer : Remote procedure call handler, listens on port 9631 for connections and services them.
...
However, in open source software such as Crowcoin Core, where everyone uses
...
pay attention to for reviewers of Crowcoin Core code.

General Crowcoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Crowcoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of Crowcoin Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against Crowcoin Core so that they can be integrated
